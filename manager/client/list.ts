import type WebSocket from "ws";
import { Client } from "./Client.js";
import type { UUID } from "../../common/index.js";
import { S2CPacket } from "../../common/packet/S2CPacket.js";
import { Packet } from "../../common/packet/Packet.js";
import type { GenericPacket } from "../../common/packet/GenericPacket.js";

const map = new Map<UUID, Client>();

function register(socket: WebSocket, userId: number): Client {
    const c = new Client(socket, userId);
    map.set(c.getUUID(), c);
    return c;
}

function unregister(c: Client): boolean;
function unregister(uuid: UUID): boolean;
function unregister(c: Client | UUID): boolean {
    let uuid = c instanceof Client ? c.getUUID() : c;
    return map.delete(uuid);
}

function get(uuid: UUID) {
    return map.get(uuid);
}

type BroadcastValue = S2CPacket | GenericPacket;
async function broadcast(handler: ((user: number) => Promise<BroadcastValue | null> | BroadcastValue | null) | BroadcastValue) {
    for (const client of map.values()) {
        const packet = handler instanceof Packet ? handler : await handler(client.getUserId());
        if (packet) {
            client.sendPacket(packet);
        }
    }
}

function disconnect(handler: (user: number) => boolean) {
    for (const client of map.values()) {
        const flag = handler(client.getUserId());
        if (flag) {
            client.closeSocket(1000);
        }
    }
}

export const ClientList = {
    register,
    unregister,
    get,
    broadcast,
    all: map,
    disconnect
} as const;
