...
export abstract class PacketReceiver {
	...
	protected async sendPacketAndReply<R extends Packet>(packet: Packet, replyClass: { new (): R; ID: string }, timeout?: number): Promise<R | null> {
		...
		if (response.id !== replyClass.ID) return null;
		return response;
	}
